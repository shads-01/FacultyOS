...
> **REINSTATED 2026-09-06:** `plan.md`'s "Auth decision" section briefly locked **anonymous** Supabase auth, then reversed back to the real email+password design this file implements — see `plan.md`'s Auth decision section and `context.md`'s "Auth" section for the current call. This file's auth design (Tasks 4-5: `@supabase/ssr` SSR clients, `middleware.js`, login/signup pages, cookie-session route handlers) is authoritative again for Arko's slice. It still only ever covers `/api/analyze` + `/api/runs`, never the 3 secondary routes — those stay Hrittika's, unauthenticated, per `Hrittika_Plan.md`, and Shads' UI (`Shads_Plan.md`) is unaffected either way. **Before executing:** this file predates both the `src/`+TypeScript scaffold and the Gemini migration — apply `context.md`'s `src/` path correction (e.g. `middleware.js` → `src/middleware.js`, `app/(auth)/login/page.js` → `src/app/(auth)/login/page.js`) and swap every Anthropic call sample below for the Gemini block in `Master_Tasklist.md` §2, same as the other three plans. Model id and env var name below (`claude-sonnet-5` / `ANTHROPIC_API_KEY`) are stale — use Gemini / `GEMINI_API_KEY`.
