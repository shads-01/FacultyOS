...
5. **`Backend_Plan.md`** — **REINSTATED 2026-09-06, not superseded.** It designs real email+password auth (`@supabase/ssr`, login/signup pages, `middleware.js` redirects) and only ever covered `/api/analyze`+`/api/runs` — that's now the authoritative auth design again, per `plan.md`'s reversed "Auth decision" section. Still apply the same `src/`-layout and Gemini corrections documented in this file when executing it (it predates both the `src/`+TS scaffold and the Gemini decision, same as the other three plans). It only covers Arko's slice (`/api/analyze`+`/api/runs`+auth pages); it never touches Hrittika's or Shads' files, so their plans are unaffected by this reversal.
...
## Auth: real email+password (reversed 2026-09-06 — was anonymous, see `plan.md`'s Auth decision)

Real Supabase Auth via `@supabase/ssr`: `/login` and `/signup` pages, session held in an HTTP-only cookie, `middleware.js` refreshing the session every request and redirecting signed-out visitors to `/login` (and signed-in visitors away from `/login`/`/signup`). `/api/analyze` and `/api/runs` route handlers read the session from cookies via `getServerSupabase()` — **no `Authorization: Bearer` header, no manual JWT verification** (that was the anonymous-auth design; it's gone). The other three routes (`/api/overlap`, `/api/grader-consistency`, `/api/grade`) still need no auth at all — this reversal only touches Arko's slice.

Concrete implementation: `Backend_Plan.md` Tasks 4-5 (SSR clients, middleware, login/signup pages, cookie-based route handlers) — reinstated, see the source-of-truth hierarchy above. Apply the `src/` path correction below to every file it creates (`app/(auth)/login/page.js` → `src/app/(auth)/login/page.js`, `middleware.js` → `src/middleware.js` since Next.js requires middleware inside `src/` when using the `src/` layout, etc.) and swap its Anthropic call for the Gemini block per the section above. In the Supabase dashboard: **Authentication → Providers → Email → turn off "Confirm email"** (so signup logs the user in immediately, no email round-trip during the demo) — replaces the old "enable anonymous sign-ins" toggle, which is no longer needed.

## Data

One table, `analysis_runs` (Exam Quality's report only — the other 3 features are stateless single-shot, no table). RLS policy `own_rows_only`: `auth.uid() = user_id`, `select`/`insert` only, no `update`/`delete`. `SUPABASE_SERVICE_ROLE_KEY` is a provisioned env var but **must never appear in app code** — the cookie-based session plus RLS is the entire trust boundary.

## ⚠️ Known deviation: the actual scaffold doesn't match what the plans assume

All three per-person plans (and the reinstated `Backend_Plan.md`) were written assuming a scaffold created with `create-next-app --no-src-dir` and plain JavaScript. The scaffold actually committed (`e2d05d3`) used the **`src/` layout with TypeScript** instead — Next.js 16.3.4, React 19.2, Tailwind 4, shadcn/ui, `@/*` aliased to `./src/*` (see `tsconfig.json`). This wasn't corrected before the plans were written, so every file path inside them needs a small mental adjustment as you execute:
...
- **`src/lib/supabase.ts`** (already committed — a browser Supabase client using the anon key) is unrelated to Arko's planned `lib/supabase/client.js` / `lib/supabase/server.js` (the `@supabase/ssr` browser/server clients used for real auth, at repo root per the point above). Both coexist; don't merge or confuse them.
- **`middleware.js` goes inside `src/`, not at repo root**, when using the `src/` layout — Next.js only picks up `src/middleware.js` in that configuration. This applies to the auth middleware reinstated from `Backend_Plan.md` (see "Auth" section above).
...
|---|---|---|---|
| **Arko** | Backend core: real email/password auth (login/signup, middleware), DB, `/api/analyze` + `/api/runs` | `Arko_Plan.md` | No — builds straight from `plan.md` |
